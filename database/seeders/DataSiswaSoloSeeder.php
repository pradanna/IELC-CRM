...
class DataSiswaSoloSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        // Load all StudyClasses for Solo branch
    ...
    private function seedActiveStudents($branchId, $leadPhaseId, $leadTypeId, $leadSourceId, $adminId, array &$studyClassMap): void
    {
        $possiblePaths = [
            ...
            base_path('docs/initiate data/solo/Data-siswa.csv'),
            base_path('docs/initiate data/solo/datasiswa.csv'),
        ...
        if (!$filePath) {
            $this->command->warn('File CSV Data Siswa Solo tidak ditemukan.');
            return;
        }

        $this->command->info("Membaca data siswa aktif Solo dari: {$filePath}");
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $city = isset($row[10]) ? trim($row[10]) : 'Surakarta';
        ...
        fclose($file);
        $this->command->info("Berhasil mengimpor {$importedCount} data siswa aktif Solo.");
    }

    private function seedStoppedStudents($branchId, $leadPhaseId, $leadTypeId, $leadSourceId, $adminId, array &$studyClassMap): void
    {
        $possiblePaths = [
            base_path('docs/initiate data/solo/siswa-stop-solo.csv'),
            base_path('docs/initiate data/solo/siswastop.csv'),
        ];
        ...
        if (!$filePath) {
            $this->command->warn('File CSV Siswa Stop Solo tidak ditemukan.');
            return;
        }

        $this->command->info("Membaca data siswa stop Solo dari: {$filePath}");
        ...
        fclose($file);
        $this->command->info("Berhasil mengimpor/memperbarui {$stopCount} data siswa stop Solo.");
    }

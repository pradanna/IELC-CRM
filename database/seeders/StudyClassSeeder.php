...
class StudyClassSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        if (!$filePath) {
            $this->command->error('File CSV kelas tidak ditemukan di folder docs/initiate data/');
            return;
        }

        $this->command->info("Membaca data kelas dari: {$filePath}");
        ...
        while (($row = fgetcsv($file)) !== false) {
            ...
            $priceMaster = $priceMasters->get($priceMasterName);

            $isSemarang = (str_contains($nameLower, 'semarang') || str_contains($nameLower, 'smg'));
            $classBranchId = ($isSemarang && $semarangBranch) ? $semarangBranch->id : $soloBranch->id;
        ...
        fclose($file);
        $this->command->info("Berhasil men-seed {$importedCount} kelas ke database.");
    }

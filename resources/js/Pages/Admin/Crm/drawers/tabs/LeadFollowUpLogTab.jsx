...
export default function LeadFollowUpLogTab({ lead, onRefresh }) {
    ...
    const openRecordModal = (defaultType = "phone") => {
        document.dispatchEvent(
            new CustomEvent("openRecordFollowUpModal", {
        ...
        <div className="flex flex-col h-full space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-3xl p-6 text-white shadow-xl shadow-slate-200/50 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
                ...
                <div className="flex flex-wrap items-center gap-2 relative z-10">
                    <button
                        ...
                        className="px-5 py-3 bg-red-600 hover:bg-red-700 text-white font-black text-xs uppercase tracking-wider rounded-2xl transition-all shadow-lg shadow-red-600/30 flex items-center gap-2 active:scale-95"
                    >
                        <Plus size={16} /> Tambah Catatan Follow-up
                    </button>
                </div>
            </div>
            ...
            <div className="flex items-center justify-between px-1">
                <div className="flex items-center gap-2">
                    ...
                    {["all", "phone", "whatsapp", "sms", "offline_chat"].map(
                        (t) => (
                            <button
                                key={t}
            ...
            <div
                ref={scrollRef}
                className="flex-1 overflow-y-auto pr-2 space-y-4 min-h-[300px] scrollbar-thin scrollbar-thumb-slate-200"
            >
                {filteredActivities.length === 0 ? (
                    <div className="flex flex-col items-center justify-center h-full py-16 text-slate-300">
                        ...
                        <p className="text-xs text-slate-400 mt-1">
                            Gunakan tombol di atas untuk mencatat percakapan.
                        </p>
                    </div>
                ) : (
                            ...
                            >
                                <div className="flex items-start justify-between mb-3">
                                    <div className="flex items-center gap-3">
                                        <div
                                            className={`p-2.5 rounded-2xl border ${cfg.badgeBg} flex items-center gap-2`}
                                        >
                                            <Icon size={16} />

...
import {
    ...
    ArrowUpDown, ArrowUp, ArrowDown, TrendingUp, AlertTriangle, FileSpreadsheet
} from 'lucide-react';
        ...
        <AdminLayout>
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 px-1">
                    ...
                    <div className="flex items-center gap-1 bg-slate-100/85 p-1 rounded-xl w-fit border border-slate-200/50 backdrop-blur-md">
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${activeMainTab === 'list'
                        ...
                        <button
                            ...
                            className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all duration-300 ${activeMainTab === 'analytics'
                                    ? 'bg-slate-900 text-white shadow-sm'
                                    : 'text-slate-500 hover:text-slate-900'
                                }`}
                        >
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50/50">
                                            ...
                                            <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] border-b border-slate-100 uppercase">Progress Report</th>
                                            <th className="px-8 py-5 border-b border-slate-100">
                                            ...
                                            students.data.length > 0 ? students.data.map((student, index) => {
                                                ...
                                                return (
                                                    <tr key={student.id} className="group hover:bg-slate-50/80 transition-colors">
                                                        <td className="px-8 py-5">
                                                            <div className="flex items-center gap-5">
                                                                <div className="relative">
                                                                    ...
                                                                    <div className={`absolute -bottom-1 -right-1 w-5 h-5 border-2 border-white rounded-full flex items-center justify-center ${student.status === 'stop' ? 'bg-rose-500' : 'bg-emerald-500'
                                                                        }`}>
                                                                        {student.status === 'stop' ? (
                                                                ...
                                                                <div className="space-y-1.5">
                                                                    ...
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        <span className={`text-[8px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded border ${student.status === 'stop'
                                                                    ...
                                                                    <div className="flex items-center flex-wrap gap-1.5 pt-0.5">
                                                                        ...
                                                                        {student.loyalty_tier && (
                                                                            <span className={`text-[9px] font-black uppercase tracking-wider px-2 py-0.5 rounded-lg border flex items-center gap-1 shadow-sm ${student.loyalty_tier.toLowerCase() === 'silver' ? 'bg-slate-50 text-slate-600 border-slate-200' :
                                                        ...
                                                        <td className="px-8 py-5">
                                                            <div className="space-y-1.5">
                                                                {student.progress_reports && student.progress_reports.length > 0 ? (
                                                                    <div className="flex flex-col gap-1">
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => openDetailModal(student)}
                                                                            className="px-2.5 py-1 bg-sky-50 hover:bg-sky-100 text-sky-700 text-[10px] font-black uppercase tracking-wider rounded-lg border border-sky-200 flex items-center gap-1.5 w-fit transition-colors"
                                                                        >
                                                                            <FileSpreadsheet size={12} className="text-sky-600 shrink-0" />
                                                                            <span>{student.progress_reports.length} Progress Report</span>
                                                                        </button>
                                                                        <span className="text-[9px] font-bold text-slate-400 truncate max-w-[160px]" title={student.progress_reports[0].title}>
                                                                            Terbaru: <span className="text-slate-600 font-extrabold">{student.progress_reports[0].title}</span>
                                                                        </span>
                                                                    </div>
                                                                ) : (
                                                                    <span className="text-[10px] font-bold text-slate-300 italic uppercase">Belum ada report</span>
                                                                )}
                                                            </div>
                                                        </td>
            ...
            {selectedStudent && (
                <StudentDetailModal
                    ...
                    student={students.data?.find(s => s.id === selectedStudent.id) || selectedStudent}
                />
            )}
            {/* Bulk Promote Modal */}
            <BulkPromoteModal
